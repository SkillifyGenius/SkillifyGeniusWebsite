...
import {
  ArrowRight, BrainCircuit, Check, ChevronRight,
  CirclePlay, Code2, HeartHandshake, Laptop, Lock, MessageSquareText,
  Shield, ShieldCheck, Sparkles, Target, UsersRound, Video, X, Zap,
} from "lucide-react";
import { Seo } from "@/components/Seo";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { programs } from "@/data/mentorship";
...
import type { BlogPost, ParentReview } from "@/types";
...
export function MentorshipHomePage() {
  const [posts, setPosts] = useState<BlogPost[]>(fallbackPosts);
  const [reviews, setReviews] = useState<ParentReview[]>([]);

  useEffect(() => {
    void Promise.all([api.getBlogPosts(), api.getReviews()]).then(([postData, reviewData]) => {
  ...
  return (
    <>
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[1.08fr_.92fr]">
          <div>
            ...
            <h1 className="mt-7 max-w-3xl font-display text-5xl font-black leading-[1.03] tracking-tight sm:text-6xl lg:text-7xl">
              Learn how to <span className="bg-gradient-to-r from-emerald-800 to-emerald-600 bg-clip-text text-transparent">think like a technologist</span>, not just write code.
            </h1>
            <p className="mt-7 max-w-xl text-lg leading-8 sm:text-xl text-[#4a635b]">
              Personal 1:1 mentorship that builds problem solving, self-learning, technical independence, responsible AI use, and cybersecurity awareness.
            </p>
            <div className="mt-9 flex flex-col gap-3 sm:flex-row sm:items-center">
              <Button size="lg" className="h-13 px-8 text-base shadow-lg shadow-emerald-950/15" asChild>
                <Link href="/trial">Book 45-min trial <ArrowRight className="ml-2 h-4 w-4" /></Link>
              </Button>
              <Button size="lg" variant="outline" className="h-13 px-7 text-base bg-white/60 hover:bg-white" asChild>
                <Link href="/programs">Explore mentorship</Link>
              </Button>
            </div>
      ...
      <div className="mx-auto max-w-7xl px-5 pb-16 sm:px-8"><Link href="/assessment" className="inline-flex items-center gap-2 rounded-xl border border-emerald-200 bg-white px-5 py-3 font-bold text-emerald-800">Create your personal roadmap <ArrowRight className="h-4 w-4" /></Link></div>
        ...
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[.7fr_1.3fr] lg:items-stretch">
          ...
          <div>
            ...
            <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:items-center">
              <Button asChild>
                <Link href="/trial">Book free 45-min assessment <ArrowRight className="h-4 w-4" /></Link>
              </Button>
              <span className="text-xs font-semibold text-muted-foreground">No obligation · Live 45-min assessment</span>
            </div>
            <Button asChild variant="ghost" className="mt-4">
              <Link href="/about">View educator profile <ChevronRight className="h-4 w-4" /></Link>
            </Button>
          </div>
        </div>
      </section>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-6 border-t border-emerald-950/15 bg-gradient-to-r from-emerald-950 via-[#0d2922] to-emerald-900 p-6 sm:p-9 text-white">
              ...
              <div className="shrink-0 w-full md:w-auto">
                <Button asChild size="lg" className="w-full md:w-auto bg-emerald-400 text-emerald-950 hover:bg-emerald-300 font-black shadow-xl shadow-emerald-400/25 h-13 px-8 text-base transition-all hover:scale-[1.02]">
                  <Link href="/trial" className="flex items-center justify-center gap-2">
      ...
      <section id="programs" className="bg-white px-5 py-20 sm:px-8 lg:py-28">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-wrap items-end justify-between gap-5">
            <div><p className="text-sm font-black uppercase tracking-[0.2em] text-emerald-700">Technology mentorship</p><h2 className="mt-4 max-w-2xl text-4xl font-black sm:text-5xl">A personal journey to technical independence.</h2></div>
            <Button variant="outline" asChild><Link href="/programs">Explore all programs <ArrowRight className="h-4 w-4" /></Link></Button>
          </div>
          <div className="mt-10 grid gap-6 lg:grid-cols-3">
            {programs.map((program) => <article key={program.id} className="flex flex-col rounded-[2rem] border border-emerald-950/10 bg-[#fbfcf8] p-7 shadow-lg shadow-emerald-950/5">
              <span className="w-fit rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-800">{program.duration}</span>
              <h3 className="mt-6 text-2xl font-black text-[#102a25]">{program.name}</h3>
              <p className="mt-4 text-sm leading-7"><strong className="text-emerald-900">For:</strong> {program.audience}</p>
              <p className="mt-4 flex-1 text-sm leading-7">{program.outcome}</p>
              <ul className="mt-6 grid gap-2 border-t border-emerald-950/10 pt-5">{program.skills.slice(0, 3).map((skill) => <li key={skill} className="flex items-center gap-2 text-sm font-semibold"><Check className="h-4 w-4 text-emerald-600" />{skill}</li>)}</ul>
              <p className="mt-5 rounded-xl bg-emerald-50 p-4 text-xs leading-6 text-emerald-950">{program.approach}</p>
              <Link href={`/assessment?program=${program.id}`} className="mt-6 inline-flex items-center gap-2 font-extrabold text-emerald-800">Build my roadmap <ArrowRight className="h-4 w-4" /></Link>
            </article>)}
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="flex items-end justify-between gap-5">
            ...
            <Button variant="ghost" asChild className="hidden sm:inline-flex">
              <Link href="/blog">All resources <ArrowRight className="h-4 w-4" /></Link>
            </Button>
          </div>
      ...
      <section className="bg-white px-5 py-20 sm:px-8 lg:py-28" id="register"><div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-8 rounded-[2rem] bg-emerald-50 p-8 sm:p-12 lg:flex-row lg:items-center"><div><p className="text-sm font-black uppercase tracking-wider text-emerald-700">Begin with clarity</p><h2 className="mt-3 text-3xl font-black text-[#102a25] sm:text-4xl">See the journey that fits your goals.</h2><p className="mt-3 max-w-2xl leading-7">Complete a short technology assessment, then review your draft roadmap with a mentor.</p></div><Button size="lg" asChild><Link href="/assessment">Build my roadmap <ArrowRight className="h-4 w-4" /></Link></Button></div></section>
    </>
  );
}
